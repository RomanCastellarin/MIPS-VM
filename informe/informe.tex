...
\Large{\bf Compilador y Máquina virtual arquitectura MIPS}\\
\vspace{5cm}
...
\item Header (12 bytes):
	\begin{itemize}
    		\item 4B : data-segment size (in Bytes)

  		\item 4B : text-segment size (in bytes)

    		\item 4B : main address
	\end{itemize}
\item Data Segment (<data-segment size>):

    Addresses start at 0x10000000

\item Text Segment (<text-segment size>):
